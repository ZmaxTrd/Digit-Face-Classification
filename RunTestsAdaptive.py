...
for i in range(1, num_grids + 1):
    ...
    path_smaller_g, expandeds, runtime_smaller_g = repeatedAdaptiveMain(grid_filename, start, goal, True) 
...
axes[0].bar(grid_indices + bar_width/2, smaller_g_runtimes, bar_width, label="Adaptive A*", color="orange")
axes[0].set_ylabel("Runtime (seconds)")
axes[0].set_title("Runtime Comparison: Forward A* vs Adaptive A*")
axes[0].legend()
...
axes[1].bar(grid_indices + bar_width/2, smaller_g_expanded, bar_width, label="Adaptive A*", color="orange")
axes[1].set_xlabel("Grid Number")
axes[1].set_ylabel("Cells Expanded")
axes[1].set_title("Cell Expansion Comparison: Forward A* vs. Adaptive A*")
axes[1].legend()
axes[1].set_xticks(grid_indices)
axes[1].set_xticklabels(grid_indices, rotation=90)
max_expanded = max(max(larger_g_expanded), max(smaller_g_expanded))# 10 y-ticks for expanded nodes
y_ticks_expanded = np.linspace(0, max_expanded, 10)
axes[1].set_yticks(y_ticks_expanded)
axes[1].grid(axis="y", linestyle="--", alpha=0.7)

plt.tight_layout()
plt.savefig("graphics/forwardAvsAdaptiveA.png")
plt.show()